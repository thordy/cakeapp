			...
			nsp.on('connection', function(client){
				...
				client.on('throw', function(data) {
					...
					match.getMatch(matchId, function(err, match) {
						...
						var playersInMatch = match.related('player2match').serialize();
						var match = match.serialize();
						new Score().addVisit(body, match, function(err, rows) {
							if (err) {
								debug('ERROR: ' + err);
								nsp.emit('error', err);
								return;
							...
							match.setCurrentPlayer(matchId, body.current_player_id, playersInMatch, function(err, match) {
								if (err) {
									debug('ERROR: ' + err);
									nsp.emit('error', err);
									return;
								}
								match.getMatch(matchId, function(err, match) { // FIXME: Don't get match again here ?
									if (err) {
										debug('ERROR: ' + err);
										nsp.emit('error', err);
										return;
